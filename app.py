...
modo_opciones = [
    t("📅 Strategic Mode (Daily - Full Year)", "📅 Modo Estratégico (Diario - Hasta 12 meses)"),
...
# Tracking para limpiar el calendario si cambiamos de modo
...
# --- LÓGICA PREVIA A LA CARGA (Solo obtener fechas límite) ---
if is_hourly:
    ...
    if 'last_month' not in st.session_state: st.session_state.last_month = selected_month
    if st.session_state.last_month != selected_month:
        need_reset = True
    ...
    min_date_val = allh_full['Day'].min().date()
    max_date_val = allh_full['Day'].max().date()
    default_dates = (min_date_val, max_date_val)
else:
    ...
    # Por defecto, carga los últimos 30 días (1 mes aprox)
...
# Si cambiamos de modo o mes, borramos el calendario para que coja el por defecto
...
selected_dates = cont_date.date_input(
    t("Select period (Max 12 months for Daily):", "Selecciona periodo (Máx 12 meses en Modo Estratégico):"), 
    value=default_dates, 
...
# --- CARGA DIFERIDA DE DATOS DIARIOS (Blindaje Extremo de RAM) ---
if not is_hourly:
    # Restringir máximo a 12 meses
    if (end_date - start_date).days > 366:
        st.sidebar.warning(t("Maximum 12 months allowed. Truncating range.", "⚠️ Máximo 12 meses permitidos. Se ha acortado el rango visualizado a 1 año exacto."))
        start_date = end_date - pd.Timedelta(days=365)
        
    # Carga EXCLUSIVAMENTE los años que abarca la selección (Máximo 2)
...
# Blindaje de columnas para ambas versiones
...
# ==============================================================================
    ...
    gc.collect()lang = cont_lang.radio("🌐 Language / Idioma", ["English", "Español"])

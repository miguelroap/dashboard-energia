...
modo_opciones = [
    ...
    t("⏱️ Operational Mode (Hourly) [DISABLED]", "⏱️ Modo Operativo (Horario) [DESHABILITADO]")
]
modo_app = cont_mode.radio("Modo", modo_opciones, label_visibility="collapsed")
is_hourly = (modo_app == modo_opciones[1])

if is_hourly:
    st.warning(t("⏳ The Operational Mode is currently disabled for maintenance. Please switch back to the Strategic Mode.", "⏳ El Modo Operativo (Horario) está temporalmente deshabilitado por mantenimiento. Por favor, selecciona el Modo Estratégico en el panel izquierdo."))
    st.stop()
...
# Como is_hourly ahora detiene la app, este bloque asume que estamos en modo diario
...
# Si cambiamos de modo, borramos el calendario para que coja el por defecto
...
# ==============================================================================
    ...
    try:
        ...
        if up_df.empty:
            ...
            # --- 4. TABLA RESUMEN NATIVA (Al final) ---
            st.markdown("---")
